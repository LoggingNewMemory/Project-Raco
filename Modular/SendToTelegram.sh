...
if [ -z "$MODULE_ID" ] || [ -z "$VERSION" ] || [ -z "$ZIP_PATH" ]; then
    echo "Usage: $0 <module_id> <version> <zip_path>"
    exit 1
fi
...
prompt_changelog() {
    if [ -n "$SENDTOTELEGRAM" ]; then
        # Default to NO changelog in non-interactive mode
        return 1
    fi
...
if prompt_telegram_post; then
    ...
    if select_telegram_groups; then
        ...
        SUMMARY_MESSAGE+="*Version:* $VERSION%0A"
        ...
        SUMMARY_MESSAGE+=%0A%0A"File uploading below..."

        upload_success=0
        upload_total=${#SELECTED_GROUPS[@]}

        # Loop through selected groups
        for i in "${!SELECTED_GROUPS[@]}"; do
            chat_id="${SELECTED_GROUPS[$i]}"
            group_name="${SELECTED_GROUP_NAMES[$i]}"
            ...
            # Upload the zip file
            if [ -f "$ZIP_PATH" ]; then
                caption="$MODULE_ID - $VERSION"

                if send_to_telegram "$ZIP_PATH" "$caption" "$chat_id"; then
                    ((upload_success++))
