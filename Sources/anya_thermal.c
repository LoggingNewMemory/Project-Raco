        ...
        while(fgets(line, sizeof(line), fp)) {
            if(strstr(line, "init.svc") && strstr(line, "thermal") && !strstr(line, "hal")) {
                ...
                if (sscanf(line, "[%127[^]]]", prop) == 1) {
                    char svc[128];
                    if (sscanf(prop, "init.svc.%s", svc) == 1) {
                        char cmd[256];
...
void disable_cpu_limits() {
    ...
    find_and_tweak_filter("/sys", "enabled", "msm_thermal", "N", 0);
    find_and_tweak_filter("/sys", "enabled", "msm_thermal", "0", 0);
...
void restore_hardware() {
    ...
    find_and_tweak_filter("/sys", "enabled", "msm_thermal", "Y", 0);
    find_and_tweak_filter("/sys", "enabled", "msm_thermal", "1", 0);
